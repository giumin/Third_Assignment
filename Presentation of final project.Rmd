...
```{r}
library(Zelig)
R3water2 <- zelig(pop_wateraccess ~ decentralization2 + log(population) + Density + log(pubexp.c) + log(consump.c) + cons_health + educ_adult + ind_pauv_mon + gini + for_dentr., data = combineddata.data, model="ls")
summary(R3water2)

R3.out <- setx(R3water2, decentralization2 = seq(0, 3, by = 2.2))

simz2 <- sim(R3water2, x = R3.out)

plot(simz2, xlab = 'decentralizatiion2', ylab = 'Water Access')
```
