...
Like decentralization, it is very challenging to find a composite proxy for health provision. We have therefore decided to focus on some key variables which data is only available on a macro level. These are: *access to water*, *access to toilet facilities*, *water provision infrastructure*, *hospital*. These indicators are from Benin's EMICoV 2010 and 2011. 

###Explanatory variables

In addition to decentralization, we use a set of variables to account for heterogeneity accross the municipalities. As so far, we have included population, population density, per capita consumption, public expenditure per capita, households health consumption, indicators on monetary poverty, literacy rate, as well as the percentage of households which favor decentralization in each municipality, all of them taken from Benin's EmiCov (2010, 2011). We assume that these variables might have some impact either on health health-related expenditures on municipal or households level, and also some impact on decentralization process per se. The basic econometric model will be further specified in the final paper.
...
R3water <- lm(pop_wateraccess ~ decentralization + population + Density + log(pubexp.c) + log(consump.c) + for_dentr. + cons_health, data = decenthealth)
summary(R3water)

R3water <- glm(pop_wateraccess ~ decentralization + Density, data = decenthealth)
summary(R3water)
